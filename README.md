...
- 📦 **アルバム・ギャラリーの一括保存と動画自動結合** — 各種メディアアルバム・ギャラリーサイト（`/a/` 形式等のアルバムページ）で、紹介用サムネイルや関連投稿、背景ぼかし画像を除外し、アルバム本来の高画質画像と動画を高精度に抽出してフォルダへ一括ダウンロードします。URL による即時検出により、ページの完全読み込み完了を待たずにツールバー上へ「一括保存」ボタンが常時現れ、右クリックメニューからも即座に保存を開始可能。さらに、設定や保存パネルから**「動画は動画で全部くっつけてからDLする」「画像だけくっつけた動画にする（1コマ3秒）」「その両方を生成する」**の3つのチェックボックスを選択可能。同梱の ffmpeg により、解像度やアスペクト比が混在していても黒帯パディング（Full HD 1080p）を付与して結合動画（`[アルバム名]_動画結合.mp4`）やスライドショー（`[アルバム名]_スライドショー.mp4`）を自動生成します（元メディアもそのまま保持されます）。
...
- ⚙️ **タブ分割された設定パネル** — 「一般・取り込み」「ブラウザ & 保存」「広告ブロック」「ログイン情報」「セキュリティ・データ」の 5 つのタブに分類され、目的の設定へ迷わずアクセスできます。ログイン情報の保持件数バッジや、Framer Motion による滑らかなピル移動（Magic Motion）を備えています。
...
│   │   │   ├── albumExtractor.ts   … アルバム・ギャラリーのメディア一括抽出（アルバム特化＋汎用）
│   │   │   ├── session.ts          … 非永続セッションと Cookie の暗号化保存
│   │   │   ├── mediaSniffer.ts     … 通信を監視して動画・音声の URL を捕捉
│   │   │   └── webviewBridge.ts    … 右クリックメニューと webview の安全設定
│   │   ├── download/
│   │   │   ├── manager.ts          … ダウンロードの実行・進捗・履歴、アルバム保存パイプライン
│   │   │   └── ffmpeg.ts           … ffmpeg の解決、引数生成、動画結合・スライドショー動画生成
...
│       ├── dialogs/                … 取り込み / 重複 / タグ一括 / 設定（タブ別コンポーネント構成）
