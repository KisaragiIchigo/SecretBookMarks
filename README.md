...
- 🔑 **ログイン情報の保管** — 内蔵ブラウザでログインすると保存するかを確認し、保存した情報はマスターパスワードから導いた鍵で**個別に暗号化**します。ヴォールトを解錠している間もパスワードは暗号のまま保持され、入力する瞬間だけ復号されます。復号した値は画面側へ渡らず、Main プロセスからページへ直接入力されます。
