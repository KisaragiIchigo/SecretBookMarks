...
- 📂 **グループの折りたたみ** — 一覧の見出しをクリックすると、そのドメイン（グループ）を折りたためます。ツールバーのボタンで一括開閉もできます。たたんだ状態はヴォールト内に暗号化して保存され、次回も引き継がれます。
...
- 🚫 **除外と個別ブロック** — サイト単位でブロックを外せます（ログインが通らない場合など）。ツールバーの盾ボタンで表示中のサイトを切り替えられるほか、設定からも編集できます。逆に、フィルターに載っていない配信元は右クリックの「このドメインをブロック」から個別に止められます。個別ブロックは除外より優先されます。
...
- 🔑 **ログイン情報の保管** — 内蔵ブラウザでログインすると保存するかを確認します（送信ボタンのクリックや Enter で送る作りにも対応。自動で聞かれない場合はツールバーの鍵ボタンから手動で保存できます）。保存した情報はマスターパスワードから導いた鍵で**個別に暗号化**します。ヴォールトを解錠している間もパスワードは暗号のまま保持され、入力する瞬間だけ復号されます。復号した値は画面側へ渡らず、Main プロセスからページへ直接入力されます。
- 🕰️ **パスワードの履歴** — 同じサイトと利用者名で保存し直すと最新のパスワードで置き換わりますが、**置き換え前のものが最大 5 件まで履歴に残ります**。打ち間違えたまま更新してしまっても、設定から以前のパスワードへ戻せます。履歴も同じ方式で暗号化されます。
...
- 🖥️ **終了と常駐** — 閉じるボタンでは常にアプリを終了します。常駐させたい場合は設定で「最小化でタスクトレイに入れる」を有効にしてください。`Ctrl+Shift+B` のクイック追加、ウィンドウ位置とサイズの保存にも対応します。
...
| `Ctrl` + `Tab` | 右のタブへ移動（右端では先頭へ戻ります） |
| `Ctrl` + `Shift` + `Tab` | 左のタブへ移動（先頭では末尾へ回ります） |
| タブを中クリック | そのタブを閉じる |
| タブを右クリック | 右側 / 左側 / 他 / 全部のタブを閉じるメニュー |
| `Ctrl` + クリック | リンクを背面のタブで開く（表示中のタブは切り替わりません） |
| `Ctrl` + `Shift` + クリック | リンクを新しいタブで開いて、そのタブへ切り替える |
