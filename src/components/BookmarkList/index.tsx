...
type RenderRow = { kind: 'group'; key: string; label: string; count: number } | { kind: 'item'; item: Bookmark }
...
export function BookmarkList({
  ...
  const rows = useMemo<RenderRow[]>(() => {
    ...
    for (const group of groups) {
      if (group.key) output.push({ kind: 'group', key: group.key, label: group.key, count: group.items.length })
    ...
    >
      {rows.slice(0, limit).map((row) =>
        row.kind === 'group' ? (
          <div
            key={`group:${row.key}`}
            className="sticky top-0 z-10 -mx-2 flex h-7 items-center gap-2 bg-ink-900/85 px-5 backdrop-blur-sm"
          >
            <span className="truncate font-display text-xs font-semibold uppercase tracking-[0.16em] text-slate-300">
              {row.label}
            </span>
